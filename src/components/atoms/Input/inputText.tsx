...
const InputText = React.forwardRef<HTMLInputElement, InputTextProps>(
  ({ className, status, label, hintMessage, leftContent, rightContent, isFullWidth = true, id, ...props }, ref) => {
    ...
    return (
      <div data-component="inputText" className={cn("flex flex-col gap-[6px]", isFullWidth ? "w-full" : "w-auto")}>
        {/* Label 섹션 */}
        {label && (
          <label htmlFor={inputId} className="text-da-b2 font-bold text-da-t-body">
            {label}
        ...
        <div className={cn(inputTextVariants({ status }), className)}>
          ...
          <input
            ...
            aria-describedby={hintMessage ? `${inputId}-hint` : undefined}
            className={cn(
        ...
        {hintMessage && (
          <p
            id={`${inputId}-hint`}
            className={cn(
              "text-da-b4 transition-colors",
              isError ? "text-da-t-attention" : "text-da-t-discription"
            )}
          >
            {hintMessage}
          </p>
